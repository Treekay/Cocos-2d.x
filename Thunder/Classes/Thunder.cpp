...
void Thunder::addEnemy(int n) {
	...
	for (unsigned i = 0; i < 3; ++i) {
		...
		double height = visibleSize.height - (50 * (i + 1));
		for (int j = 0; j < n; ++j) {
...
void Thunder::fire() {
	...
	Sprite* temp = bullet;
	bullet->runAction(
		Sequence::create(MoveBy::create(1.0f, Vec2(0, visibleSize.height)),CallFunc::create([temp, bullet, this]() {
...
void Thunder::explosion() {
	...
	explore.reserve(8);
	explore.pushBack(SpriteFrame::createWithTexture(texture, CC_RECT_PIXELS_TO_POINTS(Rect(2, 0, 190, 190))));
	for (int i = 1; i < 5; i++) {
		explore.pushBack(SpriteFrame::createWithTexture(texture, CC_RECT_PIXELS_TO_POINTS(Rect(190 * i, 0, 190, 190))));
	}
	explore.pushBack(SpriteFrame::createWithTexture(texture, CC_RECT_PIXELS_TO_POINTS(Rect(2, 190, 190, 190))));
	for (int i = 1; i < 2; i++) {
		explore.pushBack(SpriteFrame::createWithTexture(texture, CC_RECT_PIXELS_TO_POINTS(Rect(190 * i, 190, 190, 190))));
	}
...
void Thunder::meet(EventCustom * event) {
	...
	for (auto it = enemys.begin(); it != enemys.end(); it++) {
		...
		if ((*it)->getBoundingBox().getMinY() <= player->getBoundingBox().getMinY() || 
			((*it)->getBoundingBox().getMinY() <= player->getBoundingBox().getMaxY() && 
			((*it)->getBoundingBox().getMinX() <= player->getBoundingBox().getMaxX() &&
			 (*it)->getBoundingBox().getMinX() >= player->getBoundingBox().getMinX() ||
			(*it)->getBoundingBox().getMaxX() >= player->getBoundingBox().getMinX() &&
			(*it)->getBoundingBox().getMaxX() <= player->getBoundingBox().getMaxX()))) {
