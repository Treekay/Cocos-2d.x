...
bool HelloWorld::init() {
  ...
  auto gotoUpgradeScene = MenuItemFont::create("Upgrade", [](Ref *pSender) {
	  Director::getInstance()->pushScene(VersionScene::createScene());
  });
  if (gotoUpgradeScene) {
	  float x = origin.x + visibleSize.width / 2;
	  float y = origin.y + visibleSize.height - gotoUpgradeScene->getContentSize().height / 2 - 150;
	  gotoUpgradeScene->setPosition(Vec2(x, y));
  }

  // create menu, it's an autorelease object
  auto menu = Menu::create(closeItem, gotoLoginScene, gotoUsersScene, gotoModifyScene, gotoUpgradeScene, NULL);
